...
> **v2.5.0 更新**：第四板斧「源码级插桩」由 camoufox-reverse MCP v0.4.0 的 `instrument_jsvmp_source` 提供支持，在 HTTP 层改写 VMP 源码，对每个 `obj[key]` 和 `fn(args)` 插入 tap，捕获 VM 内部 switch/case 调度——是瑞数 5/6、Akamai sensor_data、webmssdk、obfuscator.io 这类"VM 自包含"场景的通用武器。详见 [`jsvmp-source-instrumentation.md`](./jsvmp-source-instrumentation.md)。
...
```
MCP 操作（推荐顺序）：
  - list_scripts → 找到异常大的 JS 文件（100KB+）
  - find_dispatch_loops(script_url=<大 JS 的 url>, min_case_count=20)   ← v0.4.0 推荐首选
...
兜底手动方式（大文件 find_dispatch_loops 超时时）：
  - search_code_in_script(script_id=N, keyword="switch") → 前后 3 行上下文
  - search_code_in_script(script_id=N, keyword="case 0:|case 1:|case 2:")
  - get_script_source(script_id=N, start_line=..., end_line=...) → 按 char_range 读取分发体
```
...
```
  ...
  │  find_dispatch_loops             │   │  instrument_jsvmp_source (新)   │
  │  trace_function(分发函数/子函数) │   │  get_instrumentation_log        │
  │  trace_property_access           │   │  → summary.hot_keys             │
  │  hook_jsvmp_interpreter (多路径) │   │  → summary.hot_methods          │
...
| VMP 自包含，算法全在 switch/case 内（瑞数 5/6、Akamai、webmssdk、obfuscator.io） | **第四板斧首选**，配合第一板斧做 I/O 边界 |
| VMP 深度绑定浏览器环境（compare_env 差异大 + 签名随环境变） | 路径 B 环境伪装（见 SKILL.md 场景 10），配第四板斧 `hot_keys` 定位指纹集 |

---

### 快速路径：v0.4.0 黄金 8 步流程（推荐先试，70%+ 瑞数/Akamai/webmssdk 场景直接搞定）

```
Step 1 — find_dispatch_loops 确认 VMP + 记下 fn_name / char_range
...
Step 4 — instrument_jsvmp_source(url_pattern="**/<VMP 文件>", mode="ast", tag="vmp1")  ← 核心新步
Step 5 — reload_with_hooks() 让所有探针先于 VMP 生效 + 清日志
Step 6 — 触发业务操作
Step 7 — get_instrumentation_log(tag_filter="vmp1", limit=300) 看 hot_keys / hot_methods / hot_functions
...
**第一板斧 — Hook 出入口**：
- [ ] `find_dispatch_loops` 确认是 VMP（case_count > 50）
...
- [ ] `freeze_prototype` 防止 VMP 覆盖 hook

**第二板斧 — 插桩解释器**：
- [ ] `hook_jsvmp_interpreter(script_url=<VMP basename>)` 多路径探针（apply/call/bind + Reflect.*/Proxy）
- [ ] 分层 `trace_function`（粗 → 中 → 细，用 `max_captures` 限制日志量）
- [ ] `trace_property_access(["navigator.*", "screen.*", ...])` 监控签名容器
...
- [ ] `instrument_jsvmp_source(url_pattern=..., mode="ast"|"regex", tag=...)` 装好
- [ ] `reload_with_hooks()` 让插桩先于 VMP 生效
- [ ] 读 `get_instrumentation_log` 三个 summary：`hot_keys` / `hot_methods` / `hot_functions`
- [ ] `hot_keys` 给出 VMP 读取的环境指纹集（top 30）
- [ ] `hot_methods` 给出 VMP 调用的方法（ObjectType.methodName 格式）
- [ ] `hot_functions` 给出 VMP 调用的函数名
- [ ] `stop_instrumentation` 完工清理
